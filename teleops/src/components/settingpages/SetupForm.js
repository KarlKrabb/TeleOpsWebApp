...
const SetupForm = (props) => {
    ...
    const workStartOptions = () => {return timeOptionsArray.slice(0, firstBreakStartIndex())}
    const firstBreakStartOptions = () => {return timeOptionsArray.slice(workStartIndex+1, firstBreakEndIndex())}
    const firstBreakEndOptions = () => {return timeOptionsArray.slice(firstBreakStartIndex+1, secondBreakStartIndex())}
    const secondBreakStartOptions = () => {return timeOptionsArray.slice(firstBreakEndIndex+1, secondBreakEndIndex())}
    const secondBreakEndOptions = () => {return timeOptionsArray.slice(secondBreakStartIndex+1,lunchBreakStartIndex())}
    const lunchStartOptions = () => {return timeOptionsArray.slice(secondBreakEndIndex+1, lunchBreakEndIndex())}
    const lunchEndOptions = () => {return timeOptionsArray.slice(lunchBreakStartIndex+1, thirdBreakStartIndex())}
    const thirdBreakStartOptions = () => {return timeOptionsArray.slice(lunchBreakEndIndex+1, thirdBreakEndIndex())}
    const thirdBreakEndOptions = () => {return timeOptionsArray.slice(thirdBreakStartIndex+1, workEndIndex())}
    const workEndOptions = () => {return timeOptionsArray.slice(thirdBreakEndIndex+1, timeOptionsArray.length)}
    ...
    const calcTotalBreakTime = () => {
        const firstBreak = firstBreakDuration && ((firstBreakDuration.label == "hours") ? (60 * firstBreakDuration.value) : firstBreakDuration.value)
        console.log(firstBreak)
        const secondBreak = secondBreakDuration && ((secondBreakDuration.label == "hours") ? (60 * secondBreakDuration.value) : secondBreakDuration.value)
        ...
        const thirdBreak = thirdBreakDuration && ((thirdBreakDuration.label == "hours") ? (60 * thirdBreakDuration.value) : thirdBreakDuration.value)
    ...
    return(
        <React.Fragment>
            <div>                
                <h3>Combined Break Time is {`${combinedBreaksDuration.hours} hours and ${combinedBreaksDuration.mins} mins`}</h3>
                <h3>Combined Work Time Is: {`${combinedWorkDuration.hours} hours and ${combinedWorkDuration.mins} mins`}</h3>
                <h3>Your Work Day Is: {`${combinedWorkWithBreaksIncluded.hours} hours and ${combinedWorkWithBreaksIncluded.mins} mins long` } </h3>
            </div>
